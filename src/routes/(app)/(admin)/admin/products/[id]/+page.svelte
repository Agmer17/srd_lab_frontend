<script lang="ts">
	...
	import { parseError, formatCurrency, resolveImg } from '$lib/api_utils.js';
	...
	async function handleSave(product: Product, originalAssignedCategories: Category[]) {
		...
		try {
			...
			for (const catId of toRemove) {
				tasks.push(
					fetch(`/api/products/categories/remove/${product.id}/${catId}`, { method: 'DELETE' })
				);
			}
...
{#await data.dataPromise}
	<!-- Loading state -->
	<div class="flex min-h-0 w-full flex-1 flex-col gap-0 overflow-y-auto">
		<header
			class="sticky top-0 z-20 flex h-[52px] shrink-0 items-center gap-3 border-b bg-background/85 px-4 backdrop-blur-md md:px-6"
		>
...
{:then res}
	{#if res.error && !res.product}
		<div class="flex h-full flex-col items-center justify-center gap-4 p-12 text-center">
			...
			<button class="sprd-btn sprd-btn--default" onclick={() => goto('/admin/products')}
		...
		>
			...
			>
				<div class="flex min-w-0 items-center gap-3">
					<button
						class="flex h-9 w-9 shrink-0 cursor-pointer items-center justify-center rounded-lg border-none bg-transparent text-muted-foreground transition-all hover:bg-secondary/20 hover:text-foreground"
						onclick={() => history.back()}
					>
						<RiArrowLeftLine class="h-5 w-5" />
					</button>
					<div class="flex min-w-0 flex-col">
						<span class="truncate text-sm font-medium tracking-tight text-foreground"
							>{product.name}</span
						>
						<span class="mt-0.5 text-[10px] leading-none text-muted-foreground">Edit Product</span>
					</div>
				</div>
				<button
			...
			<div class="mx-auto w-full max-w-3xl space-y-6 p-4 pb-16 md:p-6">
				...
				>
					...
					<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:gap-5">
						...
						<div class="col-span-2 flex flex-col gap-1.5">
							<label for="edit-slug" class="text-xs font-medium text-foreground"
						...
						<div class="flex flex-col gap-1.5">
							<label for="edit-price" class="text-xs font-medium text-foreground"
				...
				>
					...
					<p class="mb-4 text-xs text-muted-foreground">Changes will be applied when you save.</p>
				...
				>
					...
					{#if images.length === 0}
						<div
							class="mb-5 flex flex-col items-center justify-center gap-2 rounded-xl border border-dashed py-12 text-muted-foreground"
							style="border-color: oklch(0.6 0 0 / .4);"
						>
							<RiImageLine class="mb-1 h-8 w-8 opacity-40" />
								...
								>
									...
									{#if i === 0}
										<div class="absolute top-2 left-2">
											<span
												class="flex items-center gap-1 rounded-full bg-primary px-2 py-0.5 text-[10px] font-bold text-primary-foreground"
											>
									...
									<!-- Sort order badge -->
									<div
										class="absolute top-2 right-2 flex h-5 w-5 items-center justify-center rounded-full bg-black/50 text-[10px] font-bold text-white"
									>
										{i + 1}
									</div>
									<!-- Hover actions -->
									...
									>
										<div class="flex gap-1">
											<button
												class="flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg border-none bg-white/20 text-white transition-colors hover:bg-white/30"
										...
										<button
											class="flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg border-none bg-destructive/80 text-white transition-colors hover:bg-destructive"
					...
					<!-- Upload Area -->
					<div
						class="rounded-xl border border-dashed p-4 transition-colors hover:border-primary/40 md:p-5"
						style="border-color: oklch(0.6 0 0 / .4);"
					>
						<div class="flex flex-col gap-4 sm:flex-row sm:items-center">
							...
							<div class="min-w-0 flex-1">
								...
								<p class="text-xs text-muted-foreground">
									Select multiple files at once. Supported: JPG, PNG, WebP.
								</p>
							</div>
